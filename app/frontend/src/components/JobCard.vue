...
<template>
  ...
  <div
    class="relative animate-[gradient_12s_ease_infinite] rounded-2xl bg-size-[200%_200%] p-0.5"
  >
    <div
      class="rounded-2xl bg-linear-to-r from-blue-500 via-cyan-500 to-purple-500 p-px"
    >
      <!-- Inner card -->
      <Card
      ...
      >
        ...
        <CardContent class="flex h-full justify-between gap-5">
          <div
            v-if="props.job.icon"
            class="flex min-w-1/3 items-center justify-center"
          >
            <img
              :src="props.job.icon"
              alt="Company Logo"
              class="mb-4 h-50 w-auto object-contain"
            />
          </div>
          <div class="max-h-65 w-3/5 overflow-y-auto whitespace-pre-wrap">
