...
function scrollToCard(index: number, behavior: ScrollBehavior = 'smooth') {
  ...
  if (!container) return
  const children = Array.from(container.querySelectorAll('[data-card]')) as HTMLElement[]
  if (!children[index]) return

  const containerCenter = container.clientWidth / 2
  const cardCenter = children[index].offsetLeft + children[index].offsetWidth / 2
  container.scrollTo({
...
function updateScroll() {
  ...
  progress.value = (children[closestIndex].offsetLeft - first.offsetLeft) / maxScroll
...
function updateContainerHeight() {
  ...
  const cards = scrollContainer.value.querySelectorAll<HTMLElement>('[data-card]')
  let tallest = 0
  cards.forEach((card) => {
...
<template>
  <section class="relative w-full">
    ...
    >
      <!-- Left spacer -->
      <div class="w-[5%] shrink-0 sm:w-[10%] lg:w-[15%]" aria-hidden="true"></div>
      ...
      <div
        ...
        :class="index === activeIndex ? 'z-10 scale-105' : 'scale-95 opacity-80'"
      >
    ...
    <!-- Progress dots -->
    <div v-if="jobs.length > 0" class="mt-4 flex items-end justify-center gap-2">
      <button
        v-for="(job, index) in jobs"
        :key="index"
        ...
        :class="
          index === activeIndex ? 'scale-110 bg-indigo-500' : 'bg-gray-600/50 hover:bg-gray-400/70'
        "
      ></button>
    </div>
  </section>
